...
class NumberOfTripsByStopsUnitTest extends FunSuite with Matchers with OptionValues {
  ...
  private val simplePredicate: (Int) => Boolean = (length) => true
  private val maximumNumberOfStops = 5

  test("should return None for an empty graph, since there is no distance") {
    NumberOfTripsByStops("A", "B", simplePredicate, maximumNumberOfStops)(Graph.empty) shouldBe None
  }

  test("should return None if one of the cities does not exist") {
    val graph = Graph(WkDiEdge("A", "C")(2))
    NumberOfTripsByStops("A", "B", simplePredicate, maximumNumberOfStops)(graph) shouldBe None
    NumberOfTripsByStops("B", "A", simplePredicate, maximumNumberOfStops)(graph) shouldBe None
  }

  test("should return 0 if there is no route between two cities") {
    val graph = Graph(WkDiEdge("A", "B")(2), WkDiEdge("C", "E")(2))
    NumberOfTripsByStops("A", "C", simplePredicate, maximumNumberOfStops)(graph).value shouldBe 0
  }
